...
function processFile(source, config, paths, alreadyClaimed = false) {
  let claimed = source; let job; let jobId = path.basename(source).replace(/\.publish\.json$/i, 'unknown'); let lock;
  try {
    job = readJson(source); jobId = typeof job?.job_id === 'string' ? job.job_id : jobId;
    const { repo, files } = validateJob(job, config); const result = resultFile(paths, jobId);
    if (fs.existsSync(result)) { log('INFO', `job ${jobId} already has a result; skipping`); move(source, paths.done); return; }
    if (!alreadyClaimed) {
      try { claimed = move(source, paths.processing); } catch (error) { if (error.code === 'ENOENT') return; throw error; }
    }
    lock = path.join(paths.processing, `.${jobId}.lock`);
    try { fs.writeFileSync(lock, String(process.pid), { flag: 'wx' }); } catch (error) { if (error.code === 'EEXIST') { log('INFO', `job ${jobId} is already being processed`); if (!alreadyClaimed) move(claimed, paths.inbox); return; } throw error; }
    if (fs.existsSync(result)) { move(claimed, paths.done); return; }
    const clone = dedicatedClone(config, job.repo, repo);
    git(clone, ['fetch', 'origin']); git(clone, ['checkout', '--detach', `origin/${job.base_branch}`]); git(clone, ['reset', '--hard', `origin/${job.base_branch}`]); git(clone, ['clean', '-fd']); git(clone, ['checkout', '-B', job.target_branch, `origin/${job.base_branch}`]);
    ...
    if (!changed.length) { writeJson(result, { schema: RESULT_SCHEMA, job_id: jobId, status: 'no_changes', repo: job.repo, branch: job.target_branch, changed_files: [] }); move(claimed, paths.done); log('INFO', `job ${jobId} had no changes`); return; }
    git(clone, ['commit', '-m', job.commit_message]); const commitSha = git(clone, ['rev-parse', 'HEAD']); git(clone, ['push', '-u', 'origin', job.target_branch]);
    writeJson(result, { schema: RESULT_SCHEMA, job_id: jobId, status: 'success', repo: job.repo, branch: job.target_branch, commit_sha: commitSha, changed_files: changed }); move(claimed, paths.done); log('INFO', `job ${jobId} published ${commitSha}`);
  } catch (error) {
  ...
  } finally { if (lock) fs.rmSync(lock, { force: true }); }
}
function main() {
  ...
  log('INFO', `HACT Watcher v0.1 started; polling ${paths.inbox}`); scan(); setInterval(scan, config.poll_interval_ms);
}

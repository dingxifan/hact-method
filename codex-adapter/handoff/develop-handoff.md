...
> 说明：第一阶段默认"否"（Codex 只报告建议，由 CC 复核后写回并合并）是**试点期的临时保守设置**，用于在协议未验证前控制风险，**不是退回决策#24（`develop` 内置独立审查即可自合并、无需二次人工门）之前的模型**。待 Pilot C 验证 Codex 的执行结果可被 CC 低成本核验、不需要逐条重新审查后，应放开为"是"，恢复"审完即合并"的单门模型，而不是长期保留 CC 二次复核这道等价于旧 pr-review 的门。

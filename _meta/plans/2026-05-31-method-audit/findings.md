...
> - ✅ **R5 已修复（方案甲）**：13 个 task 的 structural 全部砍成纯契约（保留 前置/字段规范/主要产物/完成判据/接口约定 + 一行指向 execution 的指针），删除 工作内容/边界场景/异常处理 三段；删前把 execution 缺失的独有内容（init-project 的 _meta 语义/B类初始化/已存在文件/改名/git init 失败；deploy 的 hotfix 冲突/回滚后仍异常；draft-prd-vN 的三角分歧上报/遗漏功能走 revise-doc/跳段标注；draft-tech-design 的替换依赖/可行性风险/模板冲突；wrap-up 的 B类 cleanup；manual-test 的环境差异；develop/plan-sprint/dispatch-new 的弱独有项）逐条迁移到 execution。同步改 `_template.md`（结构层模板去三段）+ `06-gates.md:39` 悬空指针重指向 execution。init-project 边界场景 L179「暂无远端降级」与它自己工作内容「不跳过远端」自相矛盾，按 execution 强制等待行为丢弃。structural 净减 ~500 行（与 execution 去重，非删信息）。
> - ✅ **R8 已收口（冗余/TOKEN）**：03-disciplines「关于 CC 上下文管理」整节重复删除（-18）、`develop.md` 批量段压成差异表+模板（-46，最常加载 spec）、`init-project` webhook 等待两种写法合并（-~8）。**重新评估结论**：运行时只懒加载当前任务那一份 execution spec（templates/CLAUDE.md 的 `@` 在反引号内、非 import），故运行时 token 收益只在 execution 文件——已拿到。`_conventions.md` 抽 boilerplate 在懒加载下是 token 负优化（每会话多塞常加载文件），纯维护单源、有丢协议风险，**决定不做**；skeleton/04 逐 task discipline/Gate 列删除伤可读性、**不做**；目录树"漂移"多为合理 scope 差异；guide/BRIEF 单源化属非运行时维护、低 ROI。剩余维护项经用户确认**就此收口**，不强求。
...
| code-review | layer 推断表、checklist 维度、直修边界 逐字重复 | str-exec#1 |
...
| R6-3 | develop 用 `layers`(string[])，骨架/code-review 用 `layer`(单值)，命名+基数不一致 | `specs-structural/develop.md:5,28` vs `skeleton/04:40` | 统一命名与基数 |
...
| 「快速通道自检」块 | code-review/manual-test/gen-tests 3 份 | ~84 行 | token#1 |

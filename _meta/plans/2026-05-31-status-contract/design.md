...
- hact-method `code-review.md` 流程：逐条 issue 只写进 **Gitee PR comment**；仓库里唯一 CR 文件产物是 `sprint.md` 追加一句 `CR:通过/打回(原因)`（仅结论）。**仓库内不存在含结构化 issue 的文件**——「走 API」无文件可拉，否则要去解析 PR comment（正是本轮要消灭的脆弱解析）。
...
> **实现期小映射**：hact-method 现用两级 `[阻断]/[建议]`，前端/TRD 是三级 `严重/一般/建议`。改 code-review spec 时定映射（`[阻断]→严重`、`[建议]→建议`），不影响契约定义。
...
| 合并 | `code-review` | `status: merged` |
| CR 结论 | `code-review` | `code_reviews[]` |
...
> 涉及 10 份 exec spec：init-project、draft-prd-vN、draft-tech-design、plan-sprint、develop、code-review、generate-integration-tests、manual-test、dispatch-new、wrap-up-iteration。

...
HACT 当前只支持一套运行模型：`tasks/` 是唯一任务执行契约，`protocols/` 定义跨任务不变量，检查器和真实运行证据负责质量控制。旧版本兼容层、双层 specs、运行时 crossing、轮询账、恢复胶囊、投影清单和 watcher 均不再是执行要求。
...
质量控制不做减法：权威输入、固定候选、独立审查、真实验证、Gate 人签和外部效果核验继续保留。系统级 finding 必须由新的固定候选关闭，不能在原候选上用文字宣告关闭。旧项目如需采用本版，必须整体安装当前方法；不提供旧 schema、旧入口或混合运行兼容。
...
项目采用本版时必须整体安装当前 Method-owned 文件并重新验证；不提供旧版字段转换、双入口或混合运行。项目自己的业务检查、类型与安全边界不因方法文件替换而删除。

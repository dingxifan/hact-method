...
**worktree 的收尾是分发的一部分，不等合并。** `worktree remove` 只注销并删除这个检出目录，**不会删除分支或其提交**；因此临时分发分支只要已提交、推送且工作树干净，就应立刻回收，即使 PR 尚未合并。不要把“分支仍待合并”误当作保留目录的理由。

在宣布一次跨仓分发完成前，必须完成并核验以下闭环：

1. `git -C {临时目录} status --short` 必须为空；非空时先提交、推送，或明确把它作为仍在进行的工作交接，不能静默遗留。
2. 执行 `git -C {项目仓} worktree remove {临时目录}`。不得用手工删目录替代，避免留下 `.git/worktrees/` 的失效登记。
3. 同时核对 `git -C {项目仓} worktree list --porcelain` 已无该路径，且临时目录不存在；任一失败即为**分发未收尾**，应解决权限/占用问题后重试，并在交接中列明，不得报告完成。

只在“未提交的在制品”这一种情况下保留目录；其名称必须带任务或分发批次标识，交接中列出路径、分支、未提交状态和下一位负责人。分支删除另需显式授权，不能随 worktree 回收隐式执行。

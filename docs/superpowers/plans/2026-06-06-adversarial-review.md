# hact-method 对抗性审查 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 运行三角色对抗性审查 Workflow，读取 hact-method 全量方法论文件，并行输出批判性发现，合并后写入 `_meta/plans/2026-06-06-method-audit/findings.md`

**Architecture:** Phase 1 由单个 agent 读取全量 .md 文件并拼接为字符串；Phase 2 将内容注入三个独立审查 agent 并行运行（理论批评者 / 现场开发者 / AI 风险审计者）；Phase 3 合并去重排序，写入 findings.md。整个流程通过 Claude Code 的 Workflow 工具调度。

**Tech Stack:** Claude Code Workflow（JavaScript）、Glob/Read/Write tools via subagents
...
| 动作 | 路径 | 说明 |
|------|------|------|
| **Create** | `hact-method/.claude/workflows/adversarial-review.js` | Workflow 脚本，包含全部 agent 提示词 |
| **Write (by workflow)** | `hact-method/_meta/plans/2026-06-06-method-audit/findings.md` | 最终分析报告，由 workflow 内 agent 写入 |

---

## Task 1：写 Workflow 脚本

**Files:**
- Create: `E:\group-code\hact-method\.claude\workflows\adversarial-review.js`

- [ ] **Step 1.1：确认目标目录存在**

运行以下命令，若目录不存在则创建：
```powershell
New-Item -ItemType Directory -Force "E:\group-code\hact-method\.claude\workflows"
```
预期：目录存在，无报错。

- [ ] **Step 1.2：写入完整 Workflow 脚本**

创建文件 `E:\group-code\hact-method\.claude\workflows\adversarial-review.js`，内容如下：
...
export const meta = {
  ...
  phases: [
    { title: '准备文档', detail: '读取全量方法论文件' },
    { title: '对抗性审查', detail: '三个独立 agent 并行从不同角度审查' },
    { title: '合并报告', detail: '去重排序，写入 findings.md' },
  ],
}
...
const report = await agent(
  `你是合并编辑。你收到了三份独立的 hact-method 方法论对抗性审查结果，需要合并成一份统一报告。
...
await agent(
  `使用 Write 工具将以下内容写入文件 ${FINDINGS_PATH}（完全覆盖现有内容）：

${report}`,
  { label: '写入 findings.md' }
)
...
读取脚本文件开头确认 meta 块存在：
```powershell
Get-Content "E:\group-code\hact-method\.claude\workflows\adversarial-review.js" -TotalCount 10
```
预期：输出 `export const meta = {` 开头的内容。
...
**Files:**
- Trigger: Workflow tool with `scriptPath`
- Output (written by workflow): `hact-method/_meta/plans/2026-06-06-method-audit/findings.md`

- [ ] **Step 2.1：调用 Workflow 工具执行审查**

在 Claude Code 中调用 Workflow 工具：
```
Workflow({
  scriptPath: "E:\\group-code\\hact-method\\.claude\\workflows\\adversarial-review.js"
})
```
预期进度（按 phase 顺序）：
1. `准备文档`：单 agent 读取约 57 个 .md 文件，返回拼接文本
2. `对抗性审查`：三个 agent 并行，每个约 3-5 分钟
3. `合并报告`：合并 agent + 写入 agent，约 2 分钟

总耗时预期：10-15 分钟（Phase 2 并行，瓶颈是最慢的那个 agent）。

- [ ] **Step 2.2：确认 findings.md 已写入**

```powershell
Get-Content "E:\group-code\hact-method\_meta\plans\2026-06-06-method-audit\findings.md" -TotalCount 30
```
预期：输出以 `# hact-method 对抗性审查报告` 开头，包含 `## 总览` 章节。
...
**Files:**
- Review: `hact-method/_meta/plans/2026-06-06-method-audit/findings.md`
- Commit: `hact-method/.claude/workflows/adversarial-review.js` + findings.md

- [ ] **Step 3.1：读取并审阅 findings.md**

完整阅读 findings.md，确认：
- 总览统计数字已填写（非空）
- 至少有 3 条 P0 或 P1 级别发现
- 每条发现都有**定位**字段（指向具体文件 § 章节）
- 无空白占位符（如"待补充"）

若报告内容空洞或缺少具体定位，重新运行 Task 2（Workflow 支持 `resumeFromRunId` 从 Phase 2 重跑）。

- [ ] **Step 3.2：提交 workflow 脚本和 findings**

```bash
cd E:\group-code\hact-method
git add .claude/workflows/adversarial-review.js
git add _meta/plans/2026-06-06-method-audit/findings.md
git commit -m "feat: 添加对抗性审查 workflow 脚本及初版审查报告"
```
预期：commit 成功，显示 2 files changed。

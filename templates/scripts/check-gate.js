...
function checkG4(iteration, root, options = {}) {
  ...
  if (!exists(systemChecker)) fail('G4:System Verification', systemChecker, '缺 check-system-review.js，不能证明 System Verification completion');
    ...
    else pass('G4:System Verification', 'latest System Review pass 且无 open finding');
  }

  // 判据3：所有 develop(source=manual-test, iteration=vN) 已 merged
  if (tasks === null) {
    fail('G4:manual-test任务状态', statusPath, 'status.yml 不存在');
  } else {
...
function checkStaged(root) {
  ...
  for (const task of merged) {
    ...
    if (task.type === 'integration-verify') {
      ...
      inputs.add(task.latest_system_review || `iterations/${task.iteration}/system-review`);
  ...
  const run = (script, args) => {
    ...
    if (!exists(full)) { fail('当前 Method checker', script, 'checker 未安装'); return; }
  ...
  for (const task of merged.filter(item => item.type === 'integration-verify')) {
    ...
    if (!exists(systemChecker)) fail('System Verification checker', systemChecker, 'integration-verify 合并必须安装 check-system-review.js');

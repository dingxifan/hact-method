 ...
 *
 * ⚠️ **已知盲区：只能发现"本机 secrets.env 里持有的"凭据**。判据是比对真值，持有面
 *    之外一片黑——服务器 SSH 口令、数据库 root 密码、别人机器上的 key，只要不在
 *    ~/.hact/secrets.env 里，本检查器一律看不见。实测：mail-ai 的 docs/deployment-manual.md
 *    里同时有服务器 SSH 密码与 MySQL root 密码，两条都被本检查器漏掉，是 develop 的独立
 *    审查以 scope-and-secrets 维度抓到的（mail-b-026-F003）。
 *    **所以本检查器的输出是下限不是全集**，不能据它宣称"已无泄露"；人工审查那一路不可替代。
 *
 * 三条自我约束（都是今天踩过的坑换来的）：

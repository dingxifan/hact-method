...
【通用纪律 — 豁免与边界断言验两侧】
任务包 / standards / 代码注释 / 测试里凡出现「仅在 X 成立」「只有 Y 不适用」「此处可豁免」这类**边界断言**，不得只验它成立的那一侧：**X 时确实如此**与**非 X 时确实不如此**，两侧都要各撞一次；只撞一侧的断言等于没撞（它在两种世界里都绿）。一条**写错边界的豁免比一个未被发现的缺陷更有害**——缺陷只是没被找到，写错的豁免会让后面每个人在那里主动停止寻找。两侧验不了 → 判阻断，不靠"看起来合理"放行。

...
1. **强制边达标（核心 · 逐关注点穷举，禁抽样）**：对 `foundation.md`「二」表**每一行穷举一行**——不得跳过、不得"其余同上"。盲区靠"让某块显得已处理好"藏身，逐个逼问即消灭。
   ...
   - **实测档怎么判**（不信声明、读代码反推"违规写不写得出来"）：
     - **构造级** = 你**找不到**一条绕过它的代码路径。⚠️ 安全项（数据隔离/鉴权/越权）判构造级前，**主动找反例**：能否写出一个查询/接口不带作用域还能跑？能找到 → **实测档 = 人审/机械，未达构造级 → 阻断**（如 base repo 没焊死、service 仍能手写裸 where）。
     - **机械级** = 有 lint/检查会红（确认规则真存在、真接线，不是只写在 standards 里没配）。
     - **人审级** = 只靠人看。
   - **判级**：实测档 < 应有档 → 阻断；安全项未达构造级 → 阻断（最高优先）。
   - **覆盖率自证**：枚举完声明 `foundation.md 关注点数 N + 本表行数`；行数 < N = 审查未完成。

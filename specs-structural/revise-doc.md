...
**discipline**: 派生于 `target`——`prd` → `product`；`trd` / `foundation` / `project` → `architecture`
**Gate**: —
**属性**: `target` · `reason`

> 修订已确认的 PRD / TRD / Foundation / project.md 技术约束，记录原因，通知下游。
...
- **触发场景**（任一即可创建本 task）：
  - `develop` 执行中发现实现与 TRD / Foundation 有实质偏离，无法自行决策
  - `develop` 内置独立审查 loop 反复揪出阻断、根因在 TRD / Foundation 层（审查超界升级）
...
| 字段 | 类型 | 必填 | 取值 / 说明 |
|------|------|:----:|------------|
| `target` | enum | ✅ | `prd` / `trd` / `foundation` / `project`（决定 discipline 和修订对象） |
...
| 产物 | 路径 | 格式 |
|------|------|------|
| 修订后的目标文档 | `iterations/vN/prd.md` / `trd.md` / 项目根 `foundation.md` / `project.md` 技术层 | 原文件直接修改，commit 记录变更 |
...
| 受影响的 develop 任务包更新（trd/foundation 修订影响 queue 时） | `iterations/vN/queue/{task-id}.md` | 在任务包备注「已修订，请重新拾取」 |
...
| 下游 task | 交接内容 | 格式 |
|-----------|---------|------|
| `develop`（受影响的） | 更新后的任务包或 项目约束变更说明 | `iterations/vN/queue/{task-id}.md` 更新 |

...
> 标 **【linter】** 的判据由 `scripts/check-docs.js` 确定性机械核（draft-tech-design Step 5.4 跑，含 PRD↔TRD 两条交叉对账）——这就是这些判据的最终判定，TRD 不再派 subagent 冷核。未标的是**语义判据**，由签字人复核（载体真承接 AC、精化例子规格忠实 PRD 行为例子）+ pr-review 技术保真把关。
...
- [ ] 不可视区 AC 的 PRD 行为例子已精化为技术精确的可执行规格（补状态码/错误码/断言，测试脊柱幕 2），且回链的载体**真承接**对应 AC（内容真覆盖、非仅 id 在场）（语义判据，由签字人核）

...
- 全部标记 `draft-ux: 不需要` → 继续。有 UX 要求时运行 `check-ux.js vN .`，并核 prototype-map 的独立审查与用户确认是否对应当前原型；结构通过不替代浏览器走查，未完成则续做 draft-ux。

**必读文件**（可用只读调查单元并行读取并返回带路径摘要；不可用则主线顺序读）：
- `iterations/vN/prd.md`
- `iterations/vN/ux-flows.md`（如存在；作为跨功能用户任务、完成条件与恢复行为的依据）
...
**§ 交互技术方案**：按 ux-flows 的 U/S 核接口、状态和数据如何共同完成任务，尤其跨页上下文、保存/提交区别、失败与恢复。只写有技术含义的承接，不把用户任务重新拆成互不相干的功能接口。
...
**派发**：派普通档隔离审查单元，令其读 `../hact-method-lab/templates/review-briefs/trd-review.md` 按 brief 执行，只告知本期迭代版本 vN——审查单元据 brief **自读**定稿 trd.md + prd.md（+ ux-flows / prototype 如有），陌生视角逐查四维度（内部一致性 / AC 真承接 / 字段满足画面 / 覆盖完整），输出问题清单（禁 pass 盖章）。模型按项目 Codex 配置；审查维度改动去改该 brief（单一来源），不在此重述。

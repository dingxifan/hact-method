...
**§ 接口设计**：每接口用固定 header `### 接口：{方法} {路径}` 独立块，含：鉴权要求 / 请求体字段 / 响应体字段 / 错误码清单。`ux-flows.md` 存在时，每个接口注明服务于哪条流程路径（格式：`# 服务流程：{场景名}`），无对应流程路径的接口需说明原因（如"纯后端内部逻辑"）。**每个接口/模块还须注明承接哪些 PRD AC（格式：`# 满足 AC：AC-nn`，用 PRD 的全局 AC id 串链），防止 AC 在 TRD 七段中被静默丢失——AC 是 PRD 的一等内容，下游 plan-sprint 的任务包 AC 须回链到 PRD，TRD 是这条回链能否对上的中转站。** **不可视区 AC（后端/数据/逻辑）的行为例子已由 PRD 幕 1 写好**（`例：Given … / When … / Then …`）——本步是测试脊柱**幕 2：把 PRD 的行为例子精化为技术精确的可执行规格**，紧跟回链写在接口块旁：补确切状态码/错误码/断言、对齐本接口的请求/响应契约（如 PRD「应被拒绝」→ TRD「Then 返回 403 + 错误码 PERM_DENIED」）。**不创造新例子**（创造在 PRD），只做技术精化；PRD 缺某条不可视区 AC 的行为例子 → 回链对不上，回 `revise-doc(target=prd)` 或列疑点，不在 TRD 凭空补造。精化后的例子是 develop 物化成可运行测试（核心抓手 A）的规格源头，**runnable 形态守在 develop 落地，不在 TRD 预写测试文件**（守 2026-06-16「脚本/测试在代码存在后才稳」）。可视区 AC（前端视觉/交互）不精化为后端测试规格，留原型/manual-test 人走查。例子是散文约定，不进 linter 槽。 `prototype.html` 存在时，逐画面对照确认每个接口的请求/响应字段能满足该画面所需数据，避免接口返回结构与画面不匹配（前端拿不到要显示的字段）。
...
**AC 例子精化 + 覆盖映射自检**（写完七段后、报告 TRD 完成前执行）：
...
- **例子精化（幕 2）**：每条**不可视区** AC 的 PRD 行为例子已精化为技术精确的可执行规格（补确切状态码/错误码/断言、对齐接口契约）。PRD 缺行为例子的不可视区 AC → 回链对不上，回 `revise-doc(target=prd)` 补幕 1，不在 TRD 凭空创造。可视区 AC 只做覆盖映射，不精化为后端规格。**不在 TRD 落 runnable 测试文件**（runnable 物化守 develop）。
- **留人（linter 兜不住的语义残量）**：① 载体是否**真承接**该 AC（内容真覆盖，非仅 id 在场）② 精化后的例子规格是否忠实于 PRD 行为例子的意图（技术精化没夹带/没缩水）——签字时复核（见 Step 6）。

...
**§ 接口设计**：每接口含：路径 / 方法 / 鉴权要求 / 请求体字段 / 响应体字段 / 错误码清单。`ux-flows.md` 存在时，每个接口注明服务于哪条流程路径（格式：`# 服务流程：{场景名}`），无对应流程路径的接口需说明原因（如"纯后端内部逻辑"）。**每个接口/模块还须注明承接哪些 PRD AC（格式：`# 满足 AC：{功能名}·{AC 关键词}`），防止 AC 在 TRD 七段中被静默丢失——AC 是 PRD 的一等内容，下游 plan-sprint 的任务包 AC 须回链到 PRD，TRD 是这条回链能否对上的中转站。** `prototype.html` 存在时，逐画面对照确认每个接口的请求/响应字段能满足该画面所需数据，避免接口返回结构与画面不匹配（前端拿不到要显示的字段）。

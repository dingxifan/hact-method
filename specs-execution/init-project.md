...
- `scripts/review-profile.js`：内容复制自 `E:\Group-code-lab\hact-method-lab\templates\scripts\review-profile.js`（develop full review 的确定性维度选择器；`check-sprint.js --review` 同模块重算 selected/omitted，故必须与 check-sprint 一起铺设）

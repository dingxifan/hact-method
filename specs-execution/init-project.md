...
同时写入以下文件：
- **Codex 入口与配置**：复制 `templates/AGENTS.md` 到项目根并替换占位；复制 `templates/.codex/agents/*.toml` 到 `.codex/agents/`、`templates/gitee-ops.md` 到项目根 `gitee-ops.md`。不创建 CC 文件；已有入口/配置逐项合并，不覆盖。
...
- `scripts/git-truth-reader.js`：内容复制自 `templates\scripts\git-truth-reader.js`（System Verification checker 共用的 worktree/index/HEAD authoritative reader；统一 lexical/realpath/Git blob 边界）
...
node scripts/check-codex-project.js
git add .
...
| 存量项目升级 Codex 入口 | 用方法论当前 `check-codex-project.js --root .` 只读列缺口；逐仓确认后在独立 worktree 合并配置，项目自有 hook/凭据/在制品保留 |

---

## 隔离单元使用

无。全程由主线使用Codex的文件与命令能力直接执行。

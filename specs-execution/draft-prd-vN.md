...
骨架确认后**逐个功能**写完整契约，**每个写完单独确认再写下一个**。字段格式见模板 `../hact-method-lab/templates/prd.md` 功能块（`### 功能：` + 槽位，模板注释已含 AC id 与 intent/oracle/example 写法），本文只讲模板讲不了的判断：
...
🚫 每个功能（或 vN+1 的继承批量组）写完等用户确认，有改改完再下一个

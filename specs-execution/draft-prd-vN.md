...
**先决检查**：`iterations/` 目录或 `project.md` 存在 = 已初始化（`project.md` 内容为"未开始"是首期正常态，不据此判未初始化）。不存在 → 阻断：「项目未初始化，请先 init-project。」
...
| 最新 `iterations/vN/gates.md` 含 `G1: ✓` | 进 [v2+] 建新迭代目录，再读 `project.md` |
...
**加载上下文**：首期读 `_meta/input/background.md`（如有）；迭代读 `project.md`（已有现状 + 已排除功能）。
...
> **视觉规格不在本 task**：`design.md` 由 `draft-ux`（Step 1.3）生成。PRD 只在功能块标 `draft-ux: 需要/不需要`，视觉细节留 draft-ux。
...
收集各功能标记的开放问题逐条确认：有答案 → 更新功能定义；无法定 → 记 `backlog.md` 并从 PRD 移除。
...
**feedback 检查（签 G1 后）**：场景还原超 3 轮才确认 / 某类开放问题反复出现 → 写 `feedback.md`（`{日期} | {发现} | {建议}`）；无则跳过。

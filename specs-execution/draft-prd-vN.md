...
- **AC 可达性自检**（写完主体 AC 后与上表一并过）：凡 AC 里出现**容器 / 列表 / 状态名**（"在复核队列可见""处于可重抽态""进入待审清单"），当场回答两问——**该记录此刻在什么状态？从那个状态到 AC 说的位置，有没有一条边？**答不上来即 AC 悬空。这类 AC **形式上完全合格**（可独立验证、有 Given/When/Then 例子），linter 与独审都会放行，只有问可达性才现形；漏到验收才发现时，往往是那个状态窗口根本不存在、或那条边从来没人建。

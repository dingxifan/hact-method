...
- **禁止留写不出 oracle 的有行为 AC**：涉逻辑/状态/权限/数据的 AC 必须有可独立判断的 oracle；example 可省。写不出 oracle 多半是 AC 模糊，当场追问澄清。

---

> **步骤协议**：机械读取、写文件、运行检查等正常步骤静默执行，不逐步播报完成总结。只在 🚫 人类确认、会改变范围/取舍的 ⚖️ 默认判定、异常/`blocked` 和最终移交时输出；🚫 必须停下等明确回应，⚖️ 用一行结论 + 理由后继续，用户可随时推翻。
...
**v2+ 先过一遍欠账（⚖️）**：读 项目根 `backlog.md` 未勾选的 `[欠账]` 条目（上期发现、当时判「留下期」的功能缺口），逐条列给用户判「纳入本期 / 继续留 / 不做（划掉并注明）」。**没有第三种结局**——条目既不纳入也不明确放弃就会一直躺着，躺到没人记得它为什么在那。无 `[欠账]` 条目 → 一句「无遗留欠账」带过。

🚫 等用户确认场景准确（含欠账逐条去向）

### Step 2–3：方向筛选 + PRD 骨架（一次确认）

列 3–5 个解法，每个只写三角差异 **D 用户价值 / F 技术风险 / V 业务可行**、推荐度和一句理由；共享内容不重复。仅给**推荐方案**附一份一句话级 PRD 骨架（产品目标、目标用户、功能清单、MVP 排除项）；其他方案只列相对推荐方案会改变的功能范围。用户可在同一轮选别的方向并要求据差异修正这份骨架。功能超 6 个主动建议拆期并给优先级。
> 用户不认可结论 → 重过一遍，不强推；循环超 3 次记录分歧上报。

🚫 等用户一次确认方向、功能范围和排除项；用户只认可方向但改骨架时，就地改骨架，不重新展开已接受方案的三角评估。
...
骨架确认后按下面的确认分组写完整契约。字段格式见模板 `../hact-method-lab/templates/prd.md` 功能块（`### 功能：` + 槽位，模板注释已含 AC id 与 intent/oracle/example 写法），本文只讲模板讲不了的判断：

**确认分组（⚖️）**：
- v1 的清晰功能：同一业务模块、每项边界与 oracle 均明确、彼此无未裁决依赖，且不涉及权限/认证/数据隔离、金额/计费、不可逆操作或对外不可撤销副作用时，按 2–3 个一组确认。
- v1 任一功能有歧义、敏感规则、跨组依赖或用户异议 → 拆出逐个确认。
- v2+ 的**继承 / 继承·微调**成组一次确认（逐功能列一句变更点摘要，微调处高亮）；**新增 / 重构 / 简化**按上述 v1 条件决定分组或逐个。
- 用户对组内任一功能有异议，只拆该功能，不重审组内已接受项。
...
- **AC 可达性自检**（写完主体 AC 后与上表一并过）：凡 AC 里出现容器/列表/状态名，当场回答“当前状态是什么、到目标位置是否有一条边”。答不上来即 AC 悬空；不得用一个形式完整的 example 掩盖不可达 oracle。

🚫 每个确认组或单独功能写完等用户确认，有改改完再下一个
...
> **与分组/逐功能确认（Step 4）互补**：Step 4 是增量、锚定；本步仍审整份 PRD，防分组边界漏掉跨功能问题。

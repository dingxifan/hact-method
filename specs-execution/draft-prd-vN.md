...
linter（+ 待建 hook）守**格式**；**内容有效性派全新 subagent 陌生视角复核**（防同上下文自评盖章）。只喂**定稿 prd.md**（+ `_meta/input/background.md` 如有），**不喂生成对话**。审查范围只查"看得出的"，**不替用户判产品方向**：

- **内部一致性**：功能间不矛盾、`涉及实体` 引用一致、MVP 边界与功能不打架。
- **AC 可验性**：每条 AC 可独立验证、无模糊词（"友好""合理"）；有行为 AC 的例子真覆盖该 AC（非仅形式在场）。
- **覆盖完整**：用户故事覆盖核心功能；有无明显遗漏的边界 / 异常 / 空态路径。

subagent 输出**问题清单**（不是 pass 盖章）。CC 据清单与用户过一遍：真问题 → 改 PRD；属产品取舍 → 用户拍板。

> **与逐功能确认（Step 4）互补**：Step 4 是增量、锚定；本步是整份、陌生视角，逮跨功能缺口与漏网模糊 AC。"是不是用户真要的"仍归用户（design §10），subagent 不判——它只扛"可检查的遗漏 / 不一致"，与 `draft-ux` Step 3.5 同模型。

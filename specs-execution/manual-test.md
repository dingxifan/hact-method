...
> 运行时加载本文时，当前任务是提供测试环境、引导用户自主验收、收集反馈派修复任务、用户明确通过后签 G4。
> 本 task 以用户为主导，AI 负责记录、派发和跟踪，不替用户判断通过与否。
...
- **不替用户判断验收通过**：「验收通过」必须由用户明确说出，AI 不得自行宣布通过
...
存在 ux-flows 时先让用户从真实入口完成核心 U-id 任务，再按 AC 查漏；实际系统的成功、失败/取消/恢复都要有明确结果。原型通过不等于真实实现通过。复用有效集成证据，用户重点评价工作方式和体验；必要任务未通不能因单功能测试全绿签 G4。

## 会话启动

**前置检查**：读本期最新 `integration-tests/result-{日期}.md`，确认联调条件已满足：
- [ ] 所有测试场景均有结论
- [ ] 组合核对已完成，必需能力及主流程无未解决阻断；不能只凭补缝任务已派发或场景全绿进入验收
...
本期人工验收范围 —— 聚焦你能看见、能判断的部分（可视区 + 业务可接受性）：
- {可视区/业务 AC 1}
- {可视区/业务 AC 2}
...

> 不可视区 AC（后端逻辑/权限/数据/错误路径）的**正确性已由测试脊柱（develop 绿测试）+ 联调（integration-tests）机械验证**，不需要你再逐条手验——这些本来也无法靠点界面看出来（如"密码已哈希""越权被拒"）。你只管探索看得见的行为。

联调测试结论：{N} 个场景通过 / 有 {X} 个 [不阻断] 问题已记入 backlog。

本期已知欠账（开发期发现、本期无人认领，需你定夺是否本期补）：
- {[欠账] 条目 1} —— 本期补 / 留下期？
- {[欠账] 条目 2} —— 本期补 / 留下期？
{无则写「无」}
...
🚫 `[欠账]` 逐条等用户定夺，不得默认留下期：「本期补」→ 按 Step 3 派修复任务，修完在 backlog 标 `[x]`；「留下期」→ 条目留在 backlog，写进验收报告「本期已知欠账」段。**用户没表态就签 G4 = 让缺口从验收下面穿过去。**
...
- 在范围内 → 继续②
- 超出范围 → 进一步评估规模：
  - ≤3 文件且依赖层已就绪 → 建议走 B 类快速通道（`dispatch-new`），告知用户「不在本期 sprint 范围，但规模小，建议走 B 类」
  - 否则 → 写入 项目根 `backlog.md`，告知「已记入 backlog 留下期处理」，不派修复，等待下一条反馈
  - **注意**：sprint 排除项只约束 A 类 Gate 任务，不阻断 B 类
...
| 用户要求加新功能才算通过 | 告知「该需求属于下期功能，不在本期验收范围」；记入 项目根 `backlog.md`；继续当前验收 |
...
npm run type-check 2>&1 | head -10   # 或等价类型检查命令，按项目栈（如 npx tsc --noEmit）

# 前端有改动时
cd frontend && npm run build 2>&1 | tail -5
npm run type-check 2>&1 | head -10   # 或等价类型检查命令，按项目栈（如 npx vue-tsc --noEmit）
```

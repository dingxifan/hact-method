...
- 超出范围 → 进一步评估规模：
  ...
  - 否则 → 写入 `backlog.md`，告知「已记入 backlog 留下期处理」，不派修复，等待下一条反馈
...
> **完备性对账（保留全列，机械之外的人工兜底）**：本表须列全 PRD 的每一条 AC，不得遗漏——这是"开发侧任务包 AC"与"验收侧 PRD AC"两套标准对账的收口点；若某条 PRD AC 在开发链中无任务包覆盖（plan-sprint 双向对账漏网），在此暴露并记入问题记录。
> **正确性来源（不重复人工核对不可视区）**：每条 AC 的「验证来源」如实标注——**不可视区 AC 取自测试脊柱（develop 绿测试）+ 联调测试结果**（机械验过，不再人工逐条手验）；**可视区/业务 AC** 由用户人工验收。这就是乙-2 末端收口：摘掉"人在末端重复手验不可视区正确性"的冗余，人工注意力留给真正只有人能判的可视区与业务可接受性。
...
（存量项目无 `scripts/check-gate.js` → 退回 §7 G4/G5 段的人工逐条核对兜底（无 subagent），并提示补铺，见 `../hact-method/specs-execution/init-project.md` Step 3。）

...
**骨架确认后、写任务包前**，对 `依赖` 列非 `—` 的任务逐一判断，其余默认 `可并行`。
...
```
交付方式判断：
- {task-001}：串行 — 前端 {task-003} 调用其新增接口，必须先合并
- {task-002}：可并行 — 与 {task-004} 同属后端，无跨 layer 调用关系
- {task-003}：可并行 — 前端任务，等 {task-001} 合并后统一实现
继续写任务包？
```
...
> 「状态 vs 文件」分离的落点：sprint.md 是人看的视图，status.yml 是看板应用取数唯一来源；任务包正文（17 字段）不进 YAML，看板应用用到时走 API 现拉。

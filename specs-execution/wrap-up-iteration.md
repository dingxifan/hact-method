...
| **本项目具体待办的功能/技术欠账**（非规范、非方法论：能力回归 / 无人认领的交集 / Foundation 声明在代码里无落地手段） | 项目根 `backlog.md`（`[欠账]`） | 追加条目；已在 backlog 则不重复。**不得因"已记过 feedback"就删** |
| 无价值 | 直接删除 | — |
...
- **技术层**（技术选型 / 数据库结构 / 模块划分）与最终 TRD 一致；只更新本期变化，保留未改变的项目约束和验证入口，不能用本期摘要覆盖全项目技术层
- **状态标注**：

...
| 状态 | 含义 |
|---|---|
| `可取` | 在 queue 里，已获当前任务授权的执行人 都可拉 |
| `taken-by`（负责人另记 assigned_to） | 已被某 user 拉走，在做（含写代码、推 PR、等 CR、临时阻塞等任何中间情况）|
...
**临时阻塞 / 暂停 / 等待** 等中间情况都用 `taken-by` 状态 + progress 中的 free-text "阻塞原因" 字段表达，**不另立状态**。
...
| From | To | Trigger | 谁触发 |
|---|---|---|---|
| `可取` | `taken-by` | 用户拉任务（软锁，详见 §3）| 已获当前任务授权的执行人 |
...
- 冲突时**后到者重选**——共同启动协议刷新 status.yml 状态，提示"刚才那个被别人拉走了，重选一个"
- 不需要重试 / 排队 / 锁等待——直接看 status.yml 当前状态再决定
...
| 持有者发现上游 PRD/TRD/foundation 有问题，不能继续 → 改回可取 + 创建 `revise-doc` 任务 | 持有者 |
| 持有者长期无进展，dispatch 决定重派 | 已获任务重派授权的人 |

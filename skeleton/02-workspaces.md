...
| **hact-notes** | 个人积累 | `{工作区根}/hact-notes-{姓名}/`（每人独立私有仓） |
...
├── hact-notes-{username}/     个人积累仓（每人一个）
├── {项目A}/
└── {项目B}/
```
...
**物理形态**：每人一个私有仓 `hact-notes-{姓名}`，建在**团队 Gitee 组织**下（由 init-project Step4.5 自动创建，只加本人为 push 协作者），clone 到工作区根下的 `../hact-notes-{姓名}/`（与方法论仓、各项目仓同级），与项目仓同层。
...
- 同一用户上午在 hact-method 调整方法论 → 关当前会话
- 下午进项目根做 `draft-tech-design` → 开新运行时会话，新工作目录
- 晚上在同一项目根拉 `develop` 任务 → 可在同一会话继续，或开新会话

运行时入口在不同工作目录下读到对应入口文件（`AGENTS.md` 或 `AGENTS.md`）和状态文件，再执行共同启动协议并识别当前心态——这是"心态由活动驱动"的物理体现。
...
**举例**：做 `develop` 任务写代码到一半，发现 PRD 有歧义。正确做法：当前 develop 任务回退到 `[可取]` + 写阻塞理由 → 关当前会话 → 在项目根开新运行时会话拉 `revise-doc`（target=prd）→ 修订完后重新拉 develop 继续。

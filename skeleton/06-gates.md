...
注：`deploy` 在 G5 关联里不阻断签字；具体边界见 `tasks/wrap-up-iteration.md`。
...
> 适用 G1–G5。本节是 Gate 共性单一来源；Task Contract 只写 task-specific 条件。
...
所有 Gate = 对应 checker 通过 + 脚本 `🧑` 段语义残量由签字人确认。

**判官 + 门卫**：检查器查结构，`scripts/pre-commit-hook.sh` 按 staged 文件自动运行。当前 Method 缺 Node、门卫或 checker 即 FAIL，不退回旧版人工兼容；语义残量仍由签字人确认。
...
3. 项目仓缺 `scripts/check-docs.js` → 当前 Method 安装不完整，FAIL；不得退回人工兼容路径。
...
3. 项目仓缺 `scripts/check-gate.js` → 当前 Method 安装不完整，FAIL。
...
3. 项目仓缺 `scripts/check-sprint.js` 或任务包不符合当前 schema → FAIL；不得降级为人工兼容。
